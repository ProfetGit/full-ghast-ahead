#!/usr/bin/env python3
"""Validate the pack and build dist/FullGhastAhead-<version>.zip (deterministic)."""
import json
import re
import sys
import zipfile
...
PACK = ROOT / "pack"
DIST = ROOT / "dist"
NAMESPACE = "full_ghast_ahead"


def version() -> str:
    load = (PACK / "data/full_ghast_ahead/function/load.mcfunction").read_text()
    m = re.search(r'storage full_ghast_ahead:meta version set value "([^"]+)"', load)
    if not m:
...
def check_refs(errors: list[str]) -> None:
    ...
    tag = re.compile(r"#(full_ghast_ahead:[a-z0-9_./-]+)")
    for root in roots:
        for f in (root / "data").rglob("*.mcfunction"):
            ...
            for n, line in enumerate(text.splitlines(), 1):
                ...
                for is_tag, rid in ref.findall(line):
                    if not rid.startswith(NAMESPACE + ":") or ("$(" in line and rid.endswith("/")):
                        continue
                ...
                for rid in tag.findall(line):
                    if "$(" in line and rid.endswith("/"):
                        continue
                    if not any(exists(f"tags/{k}", rid, ".json") for k in ("block", "item", "function", "entity_type", "damage_type")):
...
def build(ver: str) -> Path:
    ...
    out = DIST / f"FullGhastAhead-{ver}.zip"
    for old in DIST.glob("FullGhastAhead-*.zip"):
        old.unlink()
...
def main() -> None:
    ...
    ver = version()
    for rel, needle in (("pack/pack.mcmeta", f"v{ver}"), ("pack/data/full_ghast_ahead/function/settings.mcfunction", f"v{ver}"),
                        ("pack/data/full_ghast_ahead/function/uninstall.mcfunction", f"FullGhastAhead-{ver}.zip"),
                        ("CHANGELOG.md", f"## {ver}")):
